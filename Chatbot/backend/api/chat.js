...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!rawBody.length) {
      res.statusCode = 400;
      res.setHeader("Content-Type", "application/json");
      res.end(JSON.stringify({ error: "Payload is empty" }));
      return;
    }

    const { message, history } = JSON.parse(rawBody.toString());
    if (!message) {
      res.statusCode = 400;
      res.setHeader("Content-Type", "application/json");
...
- Assignment Reminders: Active reminders are out for practical record submissions, seminar deadlines, and practical reviews. Encourage students to submit records before reviews.`;
    ...
    res.statusCode = 500;
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify({ error: error.message || "Unexpected chat backend error" }));
  }
};
